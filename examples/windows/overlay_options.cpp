...
Options options(int argc,char** argv){
    ...
    for(int i=1;i<argc;++i){std::string_view arg=argv[i];
        ...
        if(arg=="--font")result.font=value;else if(arg=="--pipe")result.pipe=value;else if(arg=="--transitions")result.file=value;else if(arg=="--report")result.report=value;
    ...
    if(result.font.empty())throw std::invalid_argument("--font <TrueType file> is required");return result;
}
